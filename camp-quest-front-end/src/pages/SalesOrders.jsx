...
const SalesOrders = () => {
  ...
  const fetchStats = async () => {
    try {
      const { data } = await axios.get('/orders/admin/orders/stats?orderType=sales');
      setStats(data.stats);
    } catch (error) {
    ...
    try {
      ...
      if (stats) {
        ...
        const statsData = [
          ...
          { label: 'Completed', value: stats.statusBreakdown?.find(s => s._id === 'completed')?.count || 0, color: [132, 204, 22] }
        ];
      ...
      autoTable(doc, {
        head: [['Order #', 'Date', 'Customer', 'Email', 'Items', 'Status', 'Priority', 'Amount']],
        body: tableData,
        startY: stats ? 73 : 50,
        theme: 'grid',
        styles: {
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-neutral-400">Manage and track sales orders</p>
        </div>
      ...
      {stats && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <StatsCard
            ...
            icon={<CheckCircle className="w-6 h-6 text-lime-500" />}
          />
        </div>
      )}
      ...
      {/* Modals - Same as RentalOrders but for sales */}
...
// Sales Order Details Modal (similar to rental but without rental-specific fields)
const SalesOrderDetailsModal = ({ order, onClose, getStatusColor, getPriorityColor }) => (
  <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
    <div className="bg-neutral-900 border border-lime-500 rounded-lg max-w-4xl w-full max-h-[70vh] overflow-y-auto">
      <div className="sticky top-0 bg-neutral-900 border-b border-neutral-700 p-6">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold text-white">Sales Order Details</h2>
          <button
            onClick={onClose}
...
// Edit Sales Order Modal (same as rental modal)
const EditSalesOrderModal = ({ order, onClose, onSuccess }) => {
  const [loading, setLoading] = useState(false);
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-neutral-900 border border-lime-500 rounded-lg max-w-2xl w-full max-h-[70vh] overflow-y-auto">
        <div className="sticky top-0 bg-neutral-900 border-b border-neutral-700 p-6">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold text-white">Edit Sales Order</h2>
            <button
              onClick={onClose}
              className="p-2 hover:bg-neutral-800 rounded-lg transition-colors"
            >
...
// Reuse StatsCard component from RentalOrders
