      ...
      <div className="md:hidden space-y-3">
        {loading ? (
          ...
          <div className="text-center py-12 bg-neutral-900 rounded-xl">
            ...
            <p className="text-neutral-400">No rental orders found</p>
          </div>
        ) : (
          orders.map((order) => (
            <RentalOrderMobileCard
              key={order._id}
              order={order}
          ...
          <table className="w-full">
            <thead className="bg-neutral-800">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-neutral-400 uppercase tracking-wider">Rental Period</th>
            ...
            <tbody className="divide-y divide-neutral-800">
              {loading ? (
                ...
                <tr>
                  <td colSpan="8" className="px-6 py-4 text-center text-neutral-400">No rental orders found</td>
                </tr>
              ) : (
                orders.map((order) => (
                  <OrderTableRow
...
const RentalOrderMobileCard = ({ order, onView, onEdit, onDelete, getStatusColor, getPriorityColor }) => {
  const rentalItem = order.items?.find(item => item.rentalDays);
  return (
    <div className="bg-neutral-900 rounded-xl border border-neutral-700 overflow-hidden">
      ...
      <div className="px-4 py-3 space-y-2">
        ...
        {rentalItem && (
          <div className="flex items-center gap-2 text-xs text-neutral-400 bg-neutral-800 rounded-lg px-3 py-2">
            <Calendar size={12} className="text-lime-500 flex-shrink-0" />
            <span>{rentalItem.rentalDays} days</span>
            {rentalItem.rentalStartDate && (
              <>
                <span>·</span>
                <span>{new Date(rentalItem.rentalStartDate).toLocaleDateString()} – {new Date(rentalItem.rentalEndDate).toLocaleDateString()}</span>
              </>
            )}
          </div>
        )}
        ...
        <div className="flex items-center gap-2">
          ...
          {order.paymentSlip?.fileUrl && (
            <span className="text-xs text-lime-500 flex items-center gap-1"><FileText size={10} /> Slip</span>
          )}
        </div>
      </div>
      {/* Action Buttons */}
